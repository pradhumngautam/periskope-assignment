...
export function Sidebar({ currentUser, chats, selectedChat, onSelectChat, loading }: SidebarProps) {
    ...
    <aside className="w-80 border-r border-gray-200 flex flex-col h-full bg-white">
                ...
                >
                  <div className="flex items-start space-x-3">
                    <Avatar className="h-10 w-10">
                      {chat.isGroup ? (
                        <span className="text-xs">{chat.name.substring(0, 2)}</span>
                      ) : (
                        <span className="text-xs">{chat.participants[0]?.full_name.substring(0, 2) || "U"}</span>
                      )}
                    </Avatar>
      ...
      <div className="border-t border-gray-200 p-2">
        <div className="grid grid-cols-6 gap-1">
          ...
          <Button variant="ghost" size="icon" className="h-10 w-10 bg-gray-100">
